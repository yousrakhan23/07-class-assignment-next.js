    ...
    <div className=" min-h-screen container flex flex-col items-center justify-center w-full mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:px-6 py-10">
        {data.map((book) => (
          <div
            key={book.id}
            className="border hover:animate-spin hover:border-[#afc93f] rounded-lg p-4 shadow-md "
          >
            <h3
              className={`${fontPirata_One.className} text-2xl font-semibold mb-2 text-white`}
            >
              {book.name}
            </h3>
